...
class EStopsPublisher:
    def __init__(self, user_estop_id: int, experimentor_estop_id: Optional[int] = None):
        ...
        if experimentor_estop_id is not None:
            ...
            self.experimentor_estop_pub = rospy.Publisher(
                "/experimentor_estop", Bool, queue_size=1
            )
    ...
    def run(self):
        while not rospy.is_shutdown():
            ...
            if self.run_experimentor_estop:
                ...
                self.experimentor_estop_pub.publish(
                    Bool(data=experimentor_estop_pressed)
                )
...
if __name__ == "__main__":
    ...
    estop_publisher = EStopsPublisher(
        user_estop_id=args.user_id, experimentor_estop_id=args.exp_id
    )
    estop_publisher.run()
