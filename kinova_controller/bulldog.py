...
class BullDog:
    def __init__(self, experimentor_estop_topic, use_user_estop, user_estop_topic):
        ...
        queue_size = 1000
        self.experimentor_emergency_stop_sub = rospy.Subscriber(
            experimentor_estop_topic,
            Bool,
            self.experimentorEmergencyStopCallback,
        ...
        if self.use_user_estop:
            self.user_emergency_stop_sub = rospy.Subscriber(
                user_estop_topic,
                Bool,
                self.userEmergencyStopCallback,
                queue_size=queue_size,
                buff_size=65536 * queue_size,
            )
    ...
    def check_status(self):
        ...
        if self.use_user_estop:
            estop_frequency_data = [
                (
                    self.user_emergency_stop_timestamps,
                    USER_ESTOP_FREQUENCY_THRESHOLD,
...
if __name__ == "__main__":
    ...
    bulldog = BullDog(
        args.experimentor_estop_topic, args.use_user_estop, args.user_estop_topic
    )
