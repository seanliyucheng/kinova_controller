...
from feeding_deployment.safety.utils import AnomalyStatus, PeekableQueue
...
class WatchDog:
    def __init__(self, estop_topic, use_user_estop, user_estop_topic):
        ...
        if use_user_estop:
            self.user_emergency_sub = rospy.Subscriber(
                user_estop_topic,
                Bool,
                self.emergencyStopCallback,
                queue_size=queue_size,
        ...
        self.watchdog_status_pub = rospy.Publisher(
            "/watchdog_status", Bool, queue_size=1
        )
    ...
    def check_status(self):
        ...
        frequencies = []
        for _queue, _threshold, _anomaly in [
            (
                self.emergency_stop_timestamps,
                ESTOP_FREQUENCY_THRESHOLD,
                AnomalyStatus.USER_ESTOP_FREQUENCY,
            ),
            (
                self.experimentor_emergency_stop_timestamps,
                EXPERIMENTOR_ESTOP_FREQUENCY_THRESHOLD,
                AnomalyStatus.EXPERIMENTOR_ESTOP_FREQUENCY,
            ),
        ]:
        ...
        if self.second_counter == WATCHDOG_RUN_FREQUENCY:
            ...
            print(
                f"Frequencies: User EStop: {frequencies[0]}, Experimentor EStop: {frequencies[1]}"
            )
            self.second_counter = 0

        for _unexpected, _anomaly in [
            (self.emergency_stop_pressed, AnomalyStatus.USER_ESTOP_PRESSED),
            (
                self.experimentor_emergency_stop_pressed,
                AnomalyStatus.EXPERIMENTOR_ESTOP_PRESSED,
            ),
        ]:
            if _unexpected:
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--estop_topic", type=str, default="/estop", help="Emergency stop topic"
    )
    args.add_argument(
        "--use_user_estop",
        type=bool,
        default=False,
        help="Use another estop that is provided to the user",
    )
    args.add_argument(
        "--user_estop_topic",
        type=str,
        default="/experimentor_estop",
        help="Experimentor emergency stop topic",
    )
